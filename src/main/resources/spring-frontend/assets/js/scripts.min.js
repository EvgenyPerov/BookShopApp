...
(function ($) {
    ...
    $(function () {
        ...
        !function (a) {
            ...
            var b, c = navigator.userAgent, d = /ikey require/i.test(c), e = /chrome/i.test(c), f = /android/i.test(c);
            a.mask = {
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="/books/' + book.id + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    ...
                    (book.isBestseller == 1 ? '<div class="Card-ribbon"><span class="Card-ribbonText">Бестселлер</span></div>' : '') +
                            ...
                            $.ajax({
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        // **************** Это вставленный мой код работы с кнопкой Показать Еще
                                        if (result.responseJSON.count < $this.data('refreshlimit') ) {
                                            var button = document.querySelector('.Cards_refresh .Cards-footer .btn');
                                            button.style.display = 'none';
                                        };
// **************** Это был мой код работы с кнопкой Показать Еще
            ...
            return {
                init: function () {
                    ...
                    $('[data-likeid]').on('click', function (e) {
                        ...
                        Login().postData('/books/rateBookReview/' + $this.data('likeid'), data, function (result) {
                            if (result.result) {
                ...
                init: function () {
                    ...
                    $('#submitPhone').on('click', function () {
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                $this.after('<div class="form-info">Вам направлен код подтверждения по SMS или ' +
                                    '4 цифры входящего ЗВОНКА или ' +
                                    'перейдите в TELEGRAM перейдите по адресу: http://t.me/BookshopApp_confirm_user_bot и введите команду /start</div>')
                    ...
                    $('#submitMail').on('click', function () {
                        ...
                        postData('/registrationEmailConfirmation', data, function (result) {
                            if (result.result) {
                        ...
                        if (!$this.hasClass('form-input_error')) {
                            postData('/approveContact', data, function (result) {
                                if (result.result) {
                                    ...
                                    $this.trigger('change');
                                    document.cookie = "tryRegist=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
                                } else {
                                    if (result) {
                                        // ******************** это вставленный мой код который работает с куки
                                        let message;
                                        let infoCountTry = 'У вас осталось попыток - ';
                                        let infoFinishTry = 'Число попыток подтверждения превышено. Через 5 минут повторите попытку, нажав "Подтвердить" для получения нового кода';
                                        let cookieLogin = document.cookie.replace(/(?:(?:^|.*;\s*)tryRegist\s*\=\s*([^;]*).*$)|^.*$/, "$1");

                                        if (cookieLogin === null || cookieLogin === '') {
                                            document.cookie = 'tryRegist=1; path=/';
                                            message = infoCountTry + '4';
                                        } else {
                                            let cookies = cookieLogin.split("/");

                                            if (cookies.length < 2) {
                                                let num = parseInt(cookies[0]);
                                                if (num < 4) {
                                                    num++;
                                                    message = infoCountTry + (5 - num);
                                                    document.cookie = 'tryRegist=' + num + '; path=/';
                                                } else {
                                                    message = infoFinishTry;
                                                    let date = new Date();
                                                    date.setMinutes(date.getMinutes() + 1);
                                                    document.cookie = 'tryRegist=5/' + date.toISOString() + '; path=/';
                                                }
                                            } else {
                                                let dateTimeFromCookie = new Date(cookies[1]);
                                                if (dateTimeFromCookie < new Date()) {
                                                    document.cookie = 'tryRegist=1; path=/';
                                                    message = infoCountTry + '4';
                                                } else {
                                                    message = infoFinishTry;
                                        ...
                                        // это был вставленный мой код который работает с куки

                                        // $row.removeClass('form-group_confirm');
                                        $this.val('');
                                        $but.prop('disabled', false);
                                        $row.find('.form-input_code').addClass('form-input_error').after('<div class="form-error">' + result.error + message + '</div>');
                                    } else {
                    ...
                    $Registration.find('.form-input_code_profile').on('focus', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'), $but = $row.find('.form-btn');
                        $but.next('.form-info').remove();
                    });
                    $Registration.find('.form-input_code_profile').on('blur', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'), data = {
                            contact: $row.find('.form-input_phone').val(),
                            code: $row.find('.form-input_code_profile').val()
                        }, $but = $row.find('.form-btn');
                        if (!$this.hasClass('form-input_error')) {
                            postData('/approveContactProfile', data, function (result) {
                                if (result.result) {
                                    $this.attr('disabled', 'disabled');
                                    $but.attr('disabled', 'disabled');
                                    $but.after('<div class="form-info">Код успешно подтвержден</div>');
                                    ...
                                    if (result) {
                                        ...
                                        $this.val('');
                                        $but.prop('disabled', false);
                                        $row.find('.form-input_code_profile').addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                                    } else {
                                        $this.addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                                    }
                                }
                    ...
                    $Registration.find('.form-input_code_profile_name').on('change', function () {
                        var $this = $(this), $row = $this.closest('.form-group'),
                            data = {
                                name: $row.find('.form-input_code_profile_name').val()
                            }, $but = $row.find('.form-btn');
                        if (!$this.hasClass('form-input_error')) {
                            postData('/changeNameProfile', data , function (result)
                            {
                                if (result.result) {
                                    $but.after('<div class="form-info">Имя изменено</div>');
                                }
                            })
                        }    ;
                    })
                            ...
                            if (!error && $this.attr('id') != 'toComeInPhone') {
                                postData('/login-by-email', $this.data('send'), function (result) {
                                    if (result.result) {
                                        ...
                                        document.cookie = "tryLoginEmail=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
                                        document.cookie = "tryLoginPhone=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
                                        if (result.error == 'ROLE_ADMIN') {
                                        ...
                                        if (result) {
                                            ...
                                            let message;
                                            let infoCountTry = 'У вас осталось попыток - ';
                                            let infoFinishTry = 'Количество попыток входа по e-mail исчерпано, попробуйте войти по телефону или повторить вход по e-mail через 5 минут, начиная с ввода e-mail';
                                            let cookieLogin = document.cookie.replace(/(?:(?:^|.*;\s*)tryLoginEmail\s*\=\s*([^;]*).*$)|^.*$/, "$1");

                                            if (cookieLogin === null || cookieLogin === '') {
                                                document.cookie = 'tryLoginEmail=1; path=/';
                                                message = infoCountTry + '4';
                                            } else {
                                                let cookies = cookieLogin.split("/");
                                                ...
                                                if (cookies.length < 2) {
                                                    ...
                                                    if (num < 4) {
                                                        num++;
                                                        message = infoCountTry + (5 - num);
                                                        document.cookie = 'tryLoginEmail=' + num + '; path=/';
                                                    } else {
                                                        message = infoFinishTry;
                                                        let date = new Date();
                                                        date.setMinutes(date.getMinutes() + 5);
                                                        document.cookie = 'tryLoginEmail=5/' + date.toISOString() + '; path=/';
                                                    }
                                                } else {
                                                    let dateTimeFromCookie = new Date(cookies[1]);
                                                    if (dateTimeFromCookie < new Date()) {
                                            ...
                                            // это был вставленный мой код который работает с куки
                                            $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">' + 'Пользователь не найден или не верный логин\/пароль или Токен недействителен ' + message  + '</div>');
                                        }
                                    }
                                })
                            } else {
                                postData('/login-by-phone-number', $this.data('send'), function (result) {
                                    if (result != null && result.result) {
                                        ...
                                        if (result) {
                                            ...
                                            let message;
                                            let infoCountTry = 'У вас осталось попыток - ';
                                            let infoFinishTry = 'Количество попыток входа по телефону исчерпано, попробуйте войти по e-mail или повторить вход по телефону через 5 минут, начиная с ввода телефона';
                                            let cookieLogin = document.cookie.replace(/(?:(?:^|.*;\s*)tryLoginPhone\s*\=\s*([^;]*).*$)|^.*$/, "$1");

                                            if (cookieLogin === null || cookieLogin === '') {
                                                document.cookie = 'tryLoginPhone=1; path=/';
                                                message = infoCountTry + '4';
                                            } else {
                                                let cookies = cookieLogin.split("/");
                                                ...
                                                if (cookies.length < 2) {
                                                    ...
                                                    if (num < 4) {
                                                        num++;
                                                        message = infoCountTry + (5 - num);
                                                        document.cookie = 'tryLoginPhone=' + num + '; path=/';
                                                    } else {
                                                        message = infoFinishTry;
                                                        let date = new Date();
                                                        date.setMinutes(date.getMinutes() + 5);
                                                        document.cookie = 'tryLoginPhone=5/' + date.toISOString() + '; path=/';
                                                    }
                                                } else {
                                                    let dateTimeFromCookie = new Date(cookies[1]);
                                                    if (dateTimeFromCookie < new Date()) {
                                                        ...
                                                        // это был вставленный мой код который работает с куки
                                            $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">' + 'Пользователь не найден или не верный код\/пароль или Токен недействителен ' + message  + '</div>');
                                        }
                                    }
                                })
                            }
                ...
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                            Login().postData('/changeBookStatus/ARCHIVED/' + $this.data('bookid'), {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && !$this.data('check')) {
                            ...
                            Login().postData('/books/changeBookStatus/' + $this.data('sendstatus')+ '/' + $this.data('bookid') + '/'+ $this.data('source'), {
                                booksIds: $this.data('bookid'),
                                status: status
                            }, function (result) {
                                if (result.result) {
                    ...
                    $ProductAssesment.find('.Rating input').on('change', function (e) {
                        ...
                        Login().postData('/books/changeBookStatus/rating/' + $this.closest('.Rating').data('bookid') +'?value='+ $this.val(), {
                                ...
                                if ($(slick.$slides[slick.$slides.length - 3]).hasClass('slick-active')) {
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            case'recommended':
                                                sendData('/books/page/recommended')
                                                break;
                                            case'recent':
            ...
            return {
                init: function () {
                    $('form.Topup').on('submit', function (e) {
                        var $this = $(this);
                        e.preventDefault();
                        Login().postData('/deposit?sum=' + $this.find('[name="sum"]').val(), {
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            ...
                            // **************** Это вставленный мой код работы с кнопкой Показать Еще
                            var button = document.querySelector('.Tabs-wrap .Transactions .Transactions-footer .btn');
                            var countOfTransaction = button.getAttribute('data-transactionlimit');

                            if (result.responseJSON.count < countOfTransaction) {
                                button.style.display = 'none';
                            }
// **************** Это был мой код работы с кнопкой Показать Еще
